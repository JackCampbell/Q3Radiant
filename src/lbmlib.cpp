...
void LoadPCX(char *filename, byte **pic, byte **palette, int *width, int *height) {
	...
	if (len == -1)
		len = PakLoadFile(filename, (void **)&raw);
	if (len == -1)
		return;
...
static void LoadBMP(const char *name, byte **pic, int *width, int *height) {
	...
	*pic = NULL;
	...
	if (length == -1) {
		length = PakLoadFile((char*)name, (void**)&buffer);
		if (length == -1) {
			return;
		}
	}
...
static void LoadPCX(const char *filename, byte **pic, byte **palette, int *width, int *height) {
	...
	*palette = NULL;
	...
	if (len == -1) {
		len = PakLoadFile((char*)filename, (void**)&raw);
		if (len == -1) {
			return;
		}
	}
	...
	*pic = out;
	...
	if (raw - (byte *)pcx > len) {
		Sys_Printf("PCX file %s was malformed\n", filename);
		free(*pic);
		*pic = NULL;
	}
...
void LoadTGA(const char *name, byte **pic, int *width, int *height) {
	...
	*pic = NULL;
	...
	if (nLen == -1) {
		nLen = PakLoadFile((char*)name, (void**)&buffer);
		if (nLen == -1) {
			return;
		}
	}
...
void LoadJPG(const char *filename, unsigned char **pic, int *width, int *height) {
	...
	if (nLen == -1) {
		nLen = PakLoadFile((char*)filename, (void**)&fbuffer);
		if (nLen == -1) {
			return;
		}
	}
...
void WriteTGA(const char *filename, byte *pixels, int width, int height, int comp) {
	// http://www.paulbourke.net/dataformats/tga/tgatest.c
	FILE *fptr;
	if ((fptr = fopen(filename, "wb")) == NULL) {
		Sys_Printf("Failed to open outputfile : %s\n", filename);
		return;
	}
	putc(0, fptr);
	putc(0, fptr);
	...
	for (int i = 0; i<height*width; i++) {
		putc(pixels[i * comp + 2], fptr);
		putc(pixels[i * comp + 1], fptr);
		putc(pixels[i * comp + 0], fptr);
		if (comp == 4) {
			putc(pixels[i * comp + 4], fptr);
		}
	}
	fclose(fptr);
}
