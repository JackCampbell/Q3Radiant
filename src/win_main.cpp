...
CStringArray g_lstCmdProcess;
int g_nProcessIndex;
char pszLine[1024];

bool BSP_StartProcess(char *lpCommandLine);
bool BSP_WaitProcess();

void BSP_ReadProcess() {
	...
	if (PeekNamedPipe(hReadPipe, NULL, 0, NULL, &dwAvail, NULL) && dwAvail > 0) {
		while (dwAvail != 0) {
			char chData[2];
			ReadFile(hReadPipe, chData, sizeof(char), &dwRead, NULL);
			if (dwRead == 0) {
				continue;
			}
			dwAvail -= dwRead;
			...
			chData[1] = '\0';
			strcat(pszLine, chData);
			if (chData[0] == '\r' || chData[0] == '\n') {
				SendMessage(g_qeglobals.d_hwndEdit, EM_REPLACESEL, 0, (LPARAM)pszLine);
				pszLine[0] = 0;
			}
		}
	}
...
bool BSP_WaitProcess() {
	...
	DWORD exitCode;
	if (GetExitCodeProcess(bsp_process, &exitCode) && exitCode != STILL_ACTIVE) {
		Sys_Printf("\nExit code: %d\n", exitCode);
		...
		CloseHandle(bsp_process);
		bsp_process = NULL;

		// if (g_nProcessIndex < g_nProcess)

		if (g_nProcessIndex < g_lstCmdProcess.GetCount()) {
			CString strCmd = g_lstCmdProcess.GetAt(g_nProcessIndex++);
			BSP_StartProcess(strCmd.GetBuffer());
			return FALSE;
		}
		return TRUE;
	}
...
bool BSP_StartProcess(char *lpCommandLine) {
	...
	BOOL ret = CreateProcess(NULL, lpCommandLine, NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &startupinfo, &ProcessInformation);
	if (!ret)
	...
	if (!bsp_process) {
		...
		Sys_Printf("Failed to create child Process: %d\n", code);
		return FALSE;
	}
...
CString QE_ExpandBspString(char *bspaction, CString strMapPath) {
	CString strToolPath = ValueForKey(g_qeglobals.d_project_entity, "rshcmd");
	CString strCmdLines;

	if (strToolPath.IsEmpty()) {
		strToolPath = g_strAppPath;
	}
	strToolPath.AppendChar('\\');
	strMapPath.Replace('/', '\\');

	char *in = ValueForKey(g_qeglobals.d_project_entity, bspaction);
	while (*in) {
		if (in[0] == '!') {
			strCmdLines.Append(strToolPath);
			in++;
			continue;
		}
		if (in[0] == '$') {
			strCmdLines.Append(strMapPath);
			in++;
			continue;
		}
		if (in[0] == '@') {
			strCmdLines.AppendChar('"');
			in++;
			continue;
		}
		strCmdLines.AppendChar(*in++);
	}
	return strCmdLines;
}


void SplitList(const char *pszText, const CString &split, CStringArray &list) {
	int start = 0, pos;

	CString strCopy = pszText;
	list.RemoveAll();
	do {
		pos = strCopy.Find(split, start);
		CString strItem;
		if (pos != -1) {
			strItem = strCopy.Mid(start, pos - start);
		} else {
			strItem = strCopy.Mid(start);
		}
		strItem.Trim();
		list.Add(strItem);
		start = pos + split.GetLength();
	} while (pos != -1);
}
...
void RunBsp(char *command) {
	...
	DWORD dwExitcode;
	BOOL ret = GetExitCodeProcess(g_hToolThread, &dwExitcode);
	...
	CString strSys = QE_ExpandBspString(command, name);
	Pointfile_Delete();
	Sys_ClearPrintf();
	Sys_Printf("======================\n");
	...
	SplitList(strSys, "&&", g_lstCmdProcess);
	g_nProcessIndex = 0;

	if (!g_lstCmdProcess.IsEmpty()) {
		CString strCmd = g_lstCmdProcess.GetAt(g_nProcessIndex++);
		BSP_StartProcess(strCmd.GetBuffer());
	}
}
...
void DLLBuildDone() {
	...
	CString strElapsed;
	strElapsed.Format("Run time was %i hours, %i minutes and %i seconds\n", tElapsed.GetHours(), tElapsed.GetMinutes(), tElapsed.GetSeconds());
...
BOOL LoadRegistryInfo(const char *pszName, void *pvBuf, long *plSize)
{
	...
	if (lres != ERROR_SUCCESS)
	{
		...
		Sys_Printf( "WARNING: RegQueryValueEx failed in LoadRegistryInfo for %s : %s\n", pszName, Message );
#endif
		return FALSE;
	}
