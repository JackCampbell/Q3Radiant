...
bool CheckForTinyBrush(brush_t* b, int n, float fSize) {
	...
	if (bTiny)
		Sys_Printf("Possible problem brush (too small) #%i\n", n);
	return bTiny;
}
...
void UniqueTargetName(CString &strName, CString strPrefix) {
	// make a unique target value
	int max_id = 0, size = strPrefix.GetLength();
	for (entity_t* e = entities.next; e != &entities; e = e->next) {
		char *pName = ValueForKey(e, "targetname");
		if (strncmp(pName, strPrefix, size) == 0) {
			int id = atoi(&pName[size]);
			max_id = Q_MAX(max_id, id);
		}
		pName = ValueForKey(e, "target");
		if (strncmp(pName, strPrefix, size) == 0) {
			int id = atoi(&pName[size]);
			max_id = Q_MAX(max_id, id);
		}
	}
	strName.Format("%s%i", strPrefix, max_id + 1);
}


void MakeEntityTargetName(entity_t *pEntity, CString &strName) {
	CString strClass = pEntity->eclass->name;
	int pos = strClass.Find('_');
	if (pos == -1) {
		pos = 0;
	} else {
		pos++;
	}
	int end = strClass.Find('_', pos);
	CString strPrefix;
	if (end == -1) {
		strPrefix = strClass.Mid(pos);
	} else {
		strPrefix = strClass.Mid(pos, end - pos);
	}
	strPrefix.AppendChar('_');
	UniqueTargetName(strName, strPrefix);
}
	...
	if (buf) {
		...
		while (1) {
			...
			if (!strcmp(ValueForKey(ent, "classname"), "worldspawn")) {
				...
				CString strTarget("");
				if (str.GetLength() > 0) {
					entity_t *pEnt = FindEntity("targetname", str.GetBuffer(0));
					if (pEnt) {
						if (!mapStr.Lookup(str, strKey)) {
							MakeEntityTargetName(pEnt, strKey);
							// UniqueTargetName(strKey);
							mapStr.SetAt(str, strKey);
						}
						strTarget = strKey;
						SetKeyValue(ent, "targetname", strTarget.GetBuffer(0));
					}
				}
				str = ValueForKey(ent, "target");
				if (str.GetLength() > 0) {
					entity_t *pEnt = FindEntity("target", str.GetBuffer(0));
					if (pEnt) {
						if (!mapStr.Lookup(str, strKey)) {
							// UniqueTargetName(strKey);
							MakeEntityTargetName(pEnt, strKey);
						...
						SetKeyValue(ent, "target", strKey.GetBuffer(0));
					}
				}
