...
public:
	...
	afx_msg void OnUpdateTerrainEdit(CCmdUI *pCmdUI);
	afx_msg void OnSelectionMoveonly();
	afx_msg void OnUpdateMoveonly(CCmdUI* pCmdUI);
	DECLARE_MESSAGE_MAP()
};
...
inline void CMainFrame::SetActiveXY(CXYWnd* p) {
	if (m_pActiveXY)
...
inline void CMainFrame::HandleKey(UINT nChar, UINT nRepCnt, UINT nFlags, bool bDown) {
	if (bDown)
