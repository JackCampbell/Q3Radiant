...
void CCamWnd::Cam_Draw() {
	...
	m_nNumTransBrushes = m_nNumEditorBrushes = m_nNumEntityBrushes = 0;
	...
	for (brush = active_brushes.next; brush != &active_brushes; brush = brush->next) {
		...
		if (brush->brush_faces->texdef.flags & SURF_EDITOR) {
		...
		if (brush->owner != world_entity) {
			//m_EntityBrushes[m_nNumEntityBrushes++] = brush;
			// continue;
		}

		if (brush->brush_faces->d_texture->bFromShader && brush->brush_faces->d_texture->fTrans != 1.0) {
			m_TransBrushes[m_nNumTransBrushes++] = brush;
		} else {
			Brush_Draw(brush);
		}
	...
	for (i = 0; i < m_nNumEntityBrushes; i++) {
		Brush_Draw(m_EntityBrushes[i]);
	}
	//
	...
	for (i = 0; i < m_nNumEditorBrushes; i++) {
		Brush_Draw(m_EditorBrushes[i]);
	}
	...
	static double render_start = -1;
	if (render_start == -1) {
		render_start = start;
	}
	m_Camera.render_time = end - render_start;

	char buffer[64];
	sprintf(buffer, "Camera Render: %.3fms", m_Camera.render_time);
