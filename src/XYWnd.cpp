...
void CXYWnd::HandleDrop() {
	...
	if (!m_mnuDrop.GetSafeHmenu()) // first time, load it up
	{
...

		{
			...
			m_mnuDrop.AppendMenu(MF_STRING, ID_EDIT_LOADPREFAB, "Load Prefab");
			m_mnuDrop.AppendMenu(MF_STRING, ID_EDIT_SAVEPREFAB, "Save Prefab");
			m_mnuDrop.AppendMenu(MF_SEPARATOR, nID++, "");
		...
		eclass_t *e;
		CMap<CString, LPCTSTR, CMenu *, CMenu *> maps;
		for (e = eclass; e; e = e->next) {
			CString strName = e->name;
			CMenu *pChild = &m_mnuDrop;

			int pos = strName.Find("_");
			if (pos != -1) {
				CString strLeft = strName.Left(pos);
				CString strRight = strName.Mid(pos + 1);

				if (!maps.Lookup(strLeft, pChild)) {
					pChild = new CMenu;
					pChild->CreateMenu();
					m_mnuDrop.AppendMenu(MF_POPUP, reinterpret_cast<unsigned int>(pChild->GetSafeHmenu()), strLeft);
					maps.SetAt(strLeft, pChild);
				}
				// strName = strRight;
			}
...
void DrawPathLines(void) {
	...
	bool		ent_strip[MAX_MAP_ENTITIES];
	...
	for (te = entities.next; te != &entities && num_entities != MAX_MAP_ENTITIES; te = te->next) {
		char *name = ValueForKey(te, "target");
		if (name[0] != 0) {
			ent_target[num_entities] = name;
			ent_entity[num_entities] = te;
			ent_strip[num_entities] = false;
			num_entities++;
		}
		name = ValueForKey(te, "master");
		if (name[0] != 0) {
			ent_target[num_entities] = name;
			ent_entity[num_entities] = te;
			ent_strip[num_entities] = true;
			num_entities++;
		}


		if (te->eclass->nShowFlags & ECLASS_MULTI_MANAGER) {
			for (epair_t *ep = te->epairs; ep; ep = ep->next) {
				if (strcmp(ep->key, "targetname") == 0) {
					continue;
				}
				if (strcmp(ep->key, "classname") == 0) {
					continue;
				}
				if (strcmp(ep->key, "origin") == 0) {
					continue;
				}
				if (strchr(ep->key, '#') != nullptr) {
					continue;
				}
				ent_target[num_entities] = ep->key;
				ent_entity[num_entities] = te;
				ent_strip[num_entities] = true;
				num_entities++;
			}
