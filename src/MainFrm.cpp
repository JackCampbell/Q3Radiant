...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_UPDATE_COMMAND_UI(ID_TERRAIN_RAISELOWERTERRAIN, OnUpdateTerrainEdit)
	...
	ON_UPDATE_COMMAND_UI(ID_SELECTION_MOVEONLY, OnUpdateMoveonly)
END_MESSAGE_MAP()
...
void CMainFrame::SetButtonMenuStates() {
	...
	if (g_qeglobals.d_project_entity) {
		...
		FillBSPMenu();
		LoadMruInReg(g_qeglobals.d_lpMruMenu, "Software\\id\\Q3Radiant\\MRU");
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) {
	...
	UINT nID = IDR_TOOLBAR1;
	if (IsGame(GAME_ID3)) {
		nID = IDR_TOOLBAR_ADVANCED;
	}
...
void CMainFrame::CreateQEChildren() {
	...
	if (AfxGetApp()->m_lpCmdLine && strlen(AfxGetApp()->m_lpCmdLine)) {
		...
		if (!bProjectLoaded) {
			CString str = g_strAppPath + "/quake.qe4";
...
void CMainFrame::OnMiscSelectentitycolor() {
	if (edit_entity) {
		...
		int ambient = 200;
		if (IsGame(GAME_HL)) {
			int fR = 255, fG = 255, fB = 128;
			if (edit_entity->eclass->nShowFlags & ECLASS_LIGHT) {
				CString strColor = ValueForKey(edit_entity, "_light");
				sscanf(strColor, "%d %d %d %d", &fR, &fG, &fB, &ambient);
			} else {
				CString strColor = ValueForKey(edit_entity, "rendercolor");
				sscanf(strColor, "%d %d %d", &fR, &fG, &fB);
			}
			VectorSet(color, fR, fG, fB);
		...
		VectorCopy(color, g_qeglobals.d_savedinfo.colors[COLOR_ENTITY]);
		if (inspector_mode == W_ENTITY && (DoColor(COLOR_ENTITY))) {
			VectorCopy(g_qeglobals.d_savedinfo.colors[COLOR_ENTITY], color);

			char buffer[128];
			if (IsGame(GAME_HL)) {
				VectorScale(color, 255.0f, color);
				if (edit_entity->eclass->nShowFlags & ECLASS_LIGHT) {
					sprintf(buffer, "%d %d %d %d", (int)color[0], (int)color[1], (int)color[2], ambient);
					::SetWindowText(hwndEnt[EntKeyField], "_light");
				} else {
					sprintf(buffer, "%d %d %d", (int)color[0], (int)color[1], (int)color[2]);
					::SetWindowText(hwndEnt[EntKeyField], "rendercolor");
				}
...
void CMainFrame::OnUpdateTerrainEdit(CCmdUI *pCmdUI) {
	pCmdUI->SetCheck(g_qeglobals.d_select_mode == sel_terraintexture || g_qeglobals.d_select_mode == sel_terrainpoint);
}
...
void CMainFrame::OnEditSaveprefab() {
	CString strPath = ValueForKey(g_qeglobals.d_project_entity, "modpath");
	if (strPath.IsEmpty()) {
...
void CMainFrame::OnUpdateMoveonly(CCmdUI* pCmdUI) {
	pCmdUI->SetCheck(g_moveOnly);
}
