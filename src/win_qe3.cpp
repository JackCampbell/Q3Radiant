...
static char szFilter[260] = "Map file (*.map, *.reg)\0*.map\0*.reg\0\0";
static char szProjectFilter[260] = "Q3Radiant project (*.qe4, *.prj)\0*.qe4\0*.prj\0\0";
...
void OpenDialog (void)
{
	...
	char *savepath = ValueForKey(g_qeglobals.d_project_entity, "modpath");
	if (savepath[0] == 0) {
		savepath = ValueForKey(g_qeglobals.d_project_entity, "basepath");
	}
	sprintf(szDirName, "%s\\maps", savepath);
...
void SaveAsDialog(bool bRegion) {
	...
	CString strPath;
	strPath.Format("%s\\maps", savepath);
...
extern bool BSP_StartProcess(char *lpCommandLine);
extern bool BSP_WaitProcess();
...
void CheckBspProcess(void) {
	...
	if (g_PrefsDlg.m_bRunQuake == TRUE  && !g_qeglobals.d_pointfile_display_list) {
		...
		CString strBasePath, strBaseName, strBaseFile = ValueForKey(g_qeglobals.d_project_entity, "modpath");
		if (strBaseFile.IsEmpty()) {
			strBaseFile = ValueForKey(g_qeglobals.d_project_entity, "basepath");
		} else {
			g_PrefsDlg.m_bSetGame = TRUE;
		}
		ExtractPath_and_Filename(strBaseFile, strBasePath, strBaseName);

		CString strMapPath, strMapFile; // .map
		ExtractPath_and_Filename(currentmap, strMapPath, strMapFile);

		CString strExeFile = ValueForKey(g_qeglobals.d_project_entity, "game_exec");
		CString strExeArgs = ValueForKey(g_qeglobals.d_project_entity, "game_args");
		{
			CString mapname = strMapFile;
			FindReplace(mapname, ".map", "");

			CString strCopyPath;
			strCopyPath.Format("%s\\maps\\%s.bsp", strBaseFile, mapname);

			CString strBspPath = currentmap;
			FindReplace(strBspPath, ".map", ".bsp");

			bool bRun = (strBspPath.CompareNoCase(strCopyPath) == 0);
			if (!bRun) {
				bRun = CopyFile(strBspPath, strCopyPath, FALSE) == TRUE;
			}
			if (bRun) {
				CString strCmdline, strBaseLine;
				if (g_PrefsDlg.m_bSetGame == TRUE) {
					if (IsGame(GAME_ID3)) {
						strBaseLine.Format(" +set fs_game %s", strBaseName);
					} else {
						strBaseLine.Format(" -game %s", strBaseName);
					}
				}
				strCmdline.Format("%s %s %s", strExeFile, strBaseLine, strExeArgs);
				strCmdline.Replace("$", mapname);
				SetCurrentDirectory(strBasePath);

				Sys_Printf("Commandline: %s\n", strCmdline);
				Sys_Printf("Work Path: %s\n", strBasePath);
