...
bool FacingVectors(entity_t *e, vec3_t forward, vec3_t right, vec3_t up);
...
void Draw_Circle(vec3_t origin, float r, int nViewType) {
	float ix = 0;
	float iy = r;
	float ig = 3 - 2 * r;
	float idgr = -6;
	...
	while (ix <= iy) {
		...
		ix++;
		if (nViewType == XY) {
			qglVertex3f(origin[0] + ix, origin[1] + iy, origin[2]);
			qglVertex3f(origin[0] - ix, origin[1] + iy, origin[2]);
			qglVertex3f(origin[0] + ix, origin[1] - iy, origin[2]);
			qglVertex3f(origin[0] - ix, origin[1] - iy, origin[2]);
			qglVertex3f(origin[0] + iy, origin[1] + ix, origin[2]);
			qglVertex3f(origin[0] - iy, origin[1] + ix, origin[2]);
			qglVertex3f(origin[0] + iy, origin[1] - ix, origin[2]);
			qglVertex3f(origin[0] - iy, origin[1] - ix, origin[2]);
		} else if (nViewType == YZ) {
			qglVertex3f(origin[0], origin[1] + ix, origin[2] + iy);
			qglVertex3f(origin[0], origin[1] - ix, origin[2] + iy);
			qglVertex3f(origin[0], origin[1] + ix, origin[2] - iy);
			qglVertex3f(origin[0], origin[1] - ix, origin[2] - iy);
			qglVertex3f(origin[0], origin[1] + iy, origin[2] + ix);
			qglVertex3f(origin[0], origin[1] - iy, origin[2] + ix);
			qglVertex3f(origin[0], origin[1] + iy, origin[2] - ix);
			qglVertex3f(origin[0], origin[1] - iy, origin[2] - ix);
		} else if (nViewType == XZ) {
			qglVertex3f(origin[0] + ix, origin[1], origin[2] + iy);
			qglVertex3f(origin[0] - ix, origin[1], origin[2] + iy);
			qglVertex3f(origin[0] + ix, origin[1], origin[2] - iy);
			qglVertex3f(origin[0] - ix, origin[1], origin[2] - iy);
			qglVertex3f(origin[0] + iy, origin[1], origin[2] + ix);
			qglVertex3f(origin[0] - iy, origin[1], origin[2] + ix);
			qglVertex3f(origin[0] + iy, origin[1], origin[2] - ix);
			qglVertex3f(origin[0] - iy, origin[1], origin[2] - ix);
		}
...
void Draw_Sphere(float radius, const vec3_t &origin, int slice, bool volume) {
	float alpha, beta; // Storage for coordinates and angles
	vec3_t s, t;
	const float gradation = Q_PI / slice;

	for (alpha = 0.0; alpha < Q_PI; alpha += gradation) {
		qglBegin(GL_TRIANGLE_STRIP);
		for (beta = 0.0; beta < 2.01 * Q_PI; beta += gradation) {
			s[0] = origin[0] + (radius * cos(beta) * sin(alpha));
			s[1] = origin[1] + (radius * sin(beta) * sin(alpha));
			s[2] = origin[2] + (radius * cos(alpha));

			t[0] = origin[0] + (radius * cos(beta) * sin(alpha + gradation));
			t[1] = origin[1] + (radius * sin(beta) * sin(alpha + gradation));
			t[2] = origin[2] + (radius * cos(alpha + gradation));
...
void Draw_Line(vec3_t start, vec3_t end, int width = 2) {
	qglColor3f(1, 1, 1);
	qglLineWidth(width);
	qglBegin(GL_LINES);
	qglVertex3fv(start);
...
void DrawBrushEntityName(brush_t *b) {
	...
	vec3_t	endpoint, tip1, tip2, tip3, tip4;
	vec3_t	start;
	float	dist = 4;

	if (FacingVectors(b->owner, forward, right, up)) {
		VectorAdd(b->mins, b->maxs, start);
		VectorScale(start, 0.5, start);
		VectorMA(start, 8, forward, endpoint);

		VectorMA(endpoint, -dist, forward, tip1);
		VectorMA(tip1, -dist, up, tip1);
		VectorMA(tip1, 2 * dist, up, tip2);

		VectorMA(endpoint, -dist, forward, tip3);
		VectorMA(tip3, -dist, right, tip3);
		VectorMA(tip3, 2 * dist, right, tip4);
	...
	if (g_qeglobals.d_savedinfo.show_names) {
		char buffer[256];
		classname = ValueForKey(b->owner, "classname");
		targetname = ValueForKey(b->owner, "targetname");
		...
		qglCallLists(strlen(buffer), GL_UNSIGNED_BYTE, buffer);
	}
}
...
void HL_RenderMode(entity_t *e, bool bIsDecal) {
	vec3_t color;
	VectorSet(color, 255, 255, 255);

	int rendermode = IntForKey(e, "rendermode", "0");
	float blend = IntForKey(e, "renderamt", "255") / 255.0f;
	GetVectorForKey(e, "rendercolor", color);
	if ((color[0] + color[1] + color[2]) == 0) {
		VectorSet(color, 255, 255, 255); // NOTE: never pass sprites with rendercolor '0 0 0' it's a stupid Valve Hammer Editor bug
	}
	enum {
		kRenderNormal,			// src
	...
	qglDisable(GL_CULL_FACE);
	qglColor4f(color[0], color[1], color[2], blend);
}

void Q1_SpriteMode(entity_t *e) {
	qglEnable(GL_BLEND);
	qglBlendFunc(GL_SRC_ALPHA, GL_ONE);
	qglDepthMask(GL_FALSE);
	qglTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
	qglEnable(GL_ALPHA_TEST);
	qglDisable(GL_CULL_FACE);
	qglColor3fv(e->eclass->color);
}

void Q3_RenderMode(entity_t *b) {
	// qglEnable(GL_BLEND);
	// qglBlendFunc(GL_SRC_ALPHA, GL_ONE);
	// qglBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
	// qglDepthMask(GL_FALSE);
...
int HL_BeamView(brush_t *b, entitymodel *model, double current_time) {
	vec3_t start, end, dir, vMin, vMax;
	float scale = IntForKey(b->owner, "BoltWidth", "20") / 2.0f;
	float move = IntForKey(b->owner, "TextureScroll", "35") * current_time;
	float radius = FloatForKey(b->owner, "Radius", "256");
	int framerate = IntForKey(b->owner, "framerate", "10");
	entity_t *start_e = FindEntityForKey(b->owner, "LightningStart");
	entity_t *end_e = FindEntityForKey(b->owner, "LightningEnd");

	VectorCopy(b->owner->origin, end);
	VectorCopy(b->owner->origin, start);
	if (start_e) {
		VectorCopy(start_e->origin, start);
	}
	
	if (b->owner->nBeamTime != 0 && b->owner->nBeamTime > (int)current_time) {
		VectorCopy(b->owner->vBeamEnd, end);
	} else {
		if (end_e) {
			VectorCopy(end_e->origin, end);
		}
		if (VectorCompare(start, end)) {
			dir[0] = (Q_random() * 2.0f) - 1.0f;
			dir[1] = (Q_random() * 2.0f) - 1.0f;
			dir[2] = (Q_random() * 2.0f) - 1.0f;
			trace_t trace = Test_Ray(start, dir, SF_ONLY_WORLD);
			if (trace.dist > radius) {
				radius = trace.dist;
			}
			VectorMA(start, radius, dir, end);
			VectorCopy(end, b->owner->vBeamEnd);
			b->owner->nBeamTime = (int)(current_time + 4.0f);
		}
	}
	
	Model_BeamView(model, start, end, scale, move, vMin, vMax);
	return int(current_time * framerate) % model->nNumTextures;
}

int HL_SpriteView(brush_t *b, entitymodel *model, double current_time) {
	vec3_t angles, vMin, vMax;
...
int Q_SpriteView(brush_t *b, entitymodel *model, double current_time) {
	vec3_t angles, vMin, vMax;
...
int HL_DecalView(brush_t *b, entitymodel *model, double current_time) {
	...
	if (!VectorCompare(b->owner->origin, b->owner->vDecalOrigin)) {
		VectorCopy(b->owner->origin, b->owner->vDecalOrigin); // reupdate
		VectorSet(b->owner->vDecalDir, 0, 0, 1);
		for (int i = 0; i < 3; i++) {
			start[i] = b->owner->origin[i] + 1.0f;
			end[i] = b->owner->origin[i] - 1.0f;
		}
		...
		VectorMA(start, trace.dist, dir, test);
		if (!VectorCompare(test, b->owner->origin)) {
			VectorSubtract(start, end, dir);
			VectorNormalize(dir);
			trace = Test_Ray(end, dir, SF_ONLY_WORLD);
			if (!trace.face) {
				return 0;
			}
			VectorMA(end, trace.dist, dir, test);
			if (!VectorCompare(test, b->owner->origin)) {
				return 0;
			}
		}
		VectorCopy(trace.face->plane.normal, b->owner->vDecalDir);
	}
	Model_DecalView(model, b->owner->vDecalDir, vMin, vMax);
	return 0;
}
...
eclass_t *HasModel(brush_t *b) {
	...
	if (b->owner != world_entity) {
		...
		if (IsGame(GAME_ID3)) {
			...
			if (strModel.IsEmpty()) {
				strModel = ValueForKey(b->owner, "model2");
			}
			if (strModel.IsEmpty()) {
			...
			} else if (b->owner->eclass->nShowFlags & ECLASS_DECAL) {
				strModel = ValueForKey(b->owner, "texture");
		...
		if (strModel.IsEmpty() && b->owner->eclass->modelpath) {
			strModel = b->owner->eclass->modelpath;
		}
		if (strModel.IsEmpty()) {
			return nullptr;
		}
		e = b->owner->md3Class;
		if (e && strcmp(e->name, strModel) == 0) {
			return e;
		}
		e = GetCachedModel(b->owner, strModel, vMin, vMax);
		if (e != NULL) {
...
void Wolf_GetModelScale(brush_t *b, vec3_t scale) {
	float scale_deg = FloatForKey(b->owner, "modelscale", "1");
	if (scale != 0) {
		scale[0] = scale[1] = scale[2] = scale_deg;
	}
	GetVectorForKey(b->owner, "modelscale_vec", scale);
...
bool PaintedModel(brush_t *b, bool bOkToTexture) {
	...
	vec3_t vMin, vMax;
	const camera_t &cam = g_pParentWnd->GetCamera()->Camera();
	...
	if (pEclass) {
		...
		if (IsGame(GAME_ET | GAME_WOLF)) {
			Wolf_GetModelScale(b, scale);
		}

#if 0
		if (IsGame(GAME_Q2) && b->owner->eclass->nShowFlags & ECLASS_TARGET_SPAWNER) {
			char *target = ValueForKey(b->owner, "target");
			if (target[0] != 0) {
				eclass_t *spawner = Eclass_ForName(target, false);
				if (!spawner->model) {
					spawner = GetCachedModel(spawner->modelpath, vMin, vMax);
				}
				pEclass = spawner;
			}
		}
		...
		while (model != NULL) {
			...
			for (i = 0; i < model->nTriCount; i++) {
				for (j = 0; j < 3; j++) {
					...
					if (deg) {
						float nx = (((x - v[0]) * c) - ((y - v[1]) * s)) + v[0];
						float ny = (((x - v[0]) * s) + ((y - v[1]) * c)) + v[1];
						x = nx, y = ny;
					}
			...
			qglEnd();
			if (g_PrefsDlg.m_nEntityShowState & ENTITY_WIREFRAME) {
...
bool FacingVectors(entity_t *e, vec3_t forward, vec3_t right, vec3_t up) {
	...
	angle = ValueForKey(e, "angles");
	if (IsGame(GAME_HL | GAME_Q2) && angle[0] != 0) {
		sscanf(angle, "%d %d %d", &pitch, &yaw, &roll);
		const char *pPitch = ValueForKey(e, "pitch");
		if (pPitch[0] != 0) {
			pitch -= atoi(pPitch);
		}
	...
	angle = ValueForKey(e, "mangle");
	if (IsGame(GAME_Q1) && e->eclass->nShowFlags & ECLASS_INTERMISSION && angle[0] != 0) {
		sscanf(angle, "%d %d %d", &pitch, &yaw, &roll);
		VectorSet(angles, pitch, yaw, roll);
		bIsSet = true;
	}
...
void Brush_DrawFacingAngle(brush_t *b, entity_t *e) {
	...
	if (!FacingVectors(e, forward, right, up)) {
		return;
	}
...
void DrawLight(brush_t *b) {
	...
	float size;
	if (IsGame(GAME_HL)) {
		int fR = 255, fG = 255, fB = 128, fA = 200;
		const char *light = ValueForKey(b->owner, "_light");
		if (light[0] != 0) {
			sscanf(light, "%d %d %d %d", &fR, &fG, &fB, &fA);
		}
		size = fA;
		VectorSet(vTriColor, fR, fG, fB);
		VectorScale(vTriColor, 1.0f / 255, vTriColor);
	} else {
		CString strColor = ValueForKey(b->owner, "_color");
		float fR = 1.0f, fG = 1.0f, fB = 1.0f;
		if (strColor.GetLength() > 0) {
			sscanf(strColor, "%f %f %f", &fR, &fG, &fB);
			VectorSet(vTriColor, fR, fG, fB);
		}
		size = FloatForKey(b->owner, "light", "300");
	}
...
#endif
	if (IsBrushSelected(b) && (b->owner->eclass->nShowFlags & ECLASS_LIGHT)) {
		vec3_t origin;
		VectorAdd(b->mins, b->maxs, origin);
		VectorScale(origin, 0.5f, origin);
		qglColor4f(vTriColor[0], vTriColor[1], vTriColor[2], 0.33);
		Draw_Sphere(size, origin, 16, true);
	}
}
	...
	for (face = b->brush_faces, order = 0; face; face = face->next, order++) {
		...
		for (i = 0; i < w->numpoints; i++) {
			...
			if (nDrawMode == cd_texture || nDrawMode == cd_light)
			{
				...
				if (b->owner != world_entity && face->texdef.flags & SURF_FLOWING) {
...
void Brush_DrawLigthXY(brush_t *b, int nViewType) {
	vec3_t vCorners[4];
	float fMid = b->mins[2] + (b->maxs[2] - b->mins[2]) / 2;
	...
	if (IsBrushSelected(b)) {
		float light = FloatForKey(b->owner, "light");
		if (light == 0) {
			light = 300;
		}
		Draw_Circle(b->owner->origin, light, nViewType);
	}
	DrawBrushEntityName(b);
}

bool IsEditorXY(entity_t *e) {
	if (!e->md3Class) {
		return false;
	}
	if (!e->md3Class->model) {
...
void Brush_DrawXY(brush_t *b, int nViewType) {
	...
	if (b->owner->eclass->fixedsize) {
		if (g_PrefsDlg.m_bNewLightDraw && (b->owner->eclass->nShowFlags & ECLASS_LIGHT)) {
			Brush_DrawLigthXY(b, nViewType);
			return;
		}
		// else if (b->owner->eclass->nShowFlags & ECLASS_MISCMODEL)
		if (!IsEditorXY(b->owner)) {
