...
trace_t Test_Ray(vec3_t origin, vec3_t dir, int flags) {
	...
	if (!(flags & SF_SELECTED_ONLY)) {
		for (brush = active_brushes.next; brush != &active_brushes; brush = brush->next) {
			...
			if ((flags & SF_ONLY_WORLD) && brush->owner != world_entity)
				continue;
...
void Select_Deselect(bool bDeselectFaces) {
	...
	if (b->mins[2] < b->maxs[2]) {
		VectorCopy(b->mins, g_qeglobals.d_new_brush_bottom);
		VectorCopy(b->maxs, g_qeglobals.d_new_brush_top);
	}
	...
	if (nCount > 0) {
		...
		for (b = selected_brushes.next; b != &selected_brushes; b = b->next)
			if (!b->owner->eclass->fixedsize) {
				...
				if (IsGame(GAME_HL) && b->owner->eclass->nShowFlags & ECLASS_DECAL) {
	...
	if (nFaceCount > 0) {
		for (int i = 0; i < nFaceCount; i++) {
			...
			if (g_qeglobals.m_bBrushPrimitMode) {
				...
				TexMatToFakeTexCoords(bp.coords, shift, &rotate, scale);
				scale[0] += static_cast<float>(x)*0.1f;
				scale[1] += static_cast<float>(y)*0.1f;
				FakeTexCoordsToTexMat(shift, rotate, scale, bp.coords);
...
void Select_Reselect() {
	...
	while (n-- > 0) {
		brush_t*b = reinterpret_cast<brush_t*>(holdArray.GetAt(n));
		Select_Brush(b);
	}
	Sys_UpdateWindows(W_ALL);
}
