...
float Lex_FloatValue(char *&src, bool rest_on_line = false);
int Lex_IntValue(char *&src, bool rest_on_line = false);
...
struct readbuf_t {
	...
	int size;
	int offset;
	template<typename type> type *GetPtr(int count = 1, bool peek = false, bool alloc = false) {
		type *value = (type *)(buffer + offset);
		int size = sizeof(type) * count;
		if (!peek) {
			offset += size;
		}
		if (alloc) {
			type *copy = (type *)qmalloc(size);
			memcpy(copy, value, size);
			return copy;
		}
		return value;
	}
	template<typename type> type GetType(int size = 1, bool peek = false) {
		type value = *(type *)(buffer + offset);
		if (!peek) {
	...
	template<typename type> type *GetOffset(int offset, int count = 1, bool peek = false, bool alloc = false) {
		if (count == 0) {
			return nullptr;
		}
		this->offset = offset;
