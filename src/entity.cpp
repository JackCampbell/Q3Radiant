...
float	FloatForKey(entity_t *ent, const char *key, const char *defs) {
	char *k = ValueForKey(ent, key, defs);
	return atof(k);
}

int IntForKey(entity_t *ent, const char *key, const char *defs) {
	char *k = ValueForKey(ent, key, defs);
	return atoi(k);
}
...
entity_t	*Entity_Parse(qboolean onlypairs, brush_t* pList) {
	...
	} else if (e->fixedsize) {	// fixed size entity
		if (ent->brushes.onext != &ent->brushes) {
...
void Entity_Write(entity_t *e, FILE *f, qboolean use_region) {
	...
	// brush position
		...
		if (e->md3Class != NULL) { // e->eclass->nShowFlags & ECLASS_MISCMODEL
			VectorCopy(e->origin, origin);
...
entity_t	*Entity_Create(eclass_t *c) {
	...
	for (epair_t *ep = c->epairs; ep; ep = ep->next) {
...
entity_t *FindEntityForKey(entity_t *e, char *pszKey) {
	char *name = ValueForKey(e, pszKey);
	if (name[0] != 0) {
		for (entity_t *pe = entities.next; pe != NULL && pe != &entities; pe = pe->next) {
			char *targetname = ValueForKey(pe, "targetname");
